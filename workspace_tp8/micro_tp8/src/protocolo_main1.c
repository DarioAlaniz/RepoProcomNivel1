...
//-------------------------------------------------------------------------------
		...
		case 'w':
			if(datos_rec[1]==3){
				...
				switch(datos_rec[2]){
				case 1://pido BER
					gpio_w = gpio_w_c + 1;		//pido la parte baja del BER
					write();
					Q_ber_l = XGpio_DiscreteRead(&GpioInput, 1); 	//guardo la parte baja del BER
					gpio_w = gpio_w_c + 2;		//pido la parte alta del BER
					write();
					Q_ber_h = XGpio_DiscreteRead(&GpioInput, 1);	//guardo la parte alta del BER
					Ber_Q = Q_ber_h + Q_ber_l;
					send(Ber_Q,56);									//mando los 64
					break;
				case 2:
					Q_ber_h = XGpio_DiscreteRead(&GpioInput, 1);
					//send(Q_ber_h);
					break;
				case 4:
					Q_error_l = XGpio_DiscreteRead(&GpioInput, 1);
					//send(Q_error_l);
					break;
				case 8:
					Q_error_h = XGpio_DiscreteRead(&GpioInput, 1);
					//send(Q_error_h);
					break;
				}
			}
